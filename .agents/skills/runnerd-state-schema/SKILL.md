...
- Keep compatibility backfills narrow and explicit.
...
3. For schema edits, update the model tags first. Add a narrow `migrateLegacySchemaColumns` backfill only if old databases cannot safely migrate through `AutoMigrate` alone.
