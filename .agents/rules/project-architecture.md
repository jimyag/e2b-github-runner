...
- Global Runner Specs are admin API/UI data, while `/user/runner-specs` exposes only account or manageable Organization scoped Runner Types. Internal Runner Groups and Repository Policies were removed completely in Release C: their APIs and state models no longer exist, no current feature or recovery path may depend on legacy database artifacts, and they must never participate in admission.
- The ordinary-user catalog exposes raw `template_id` and `runner_group` only for the selected scope's own `scoped_custom` entries. Managed entries use the stable public template name, and `platform_custom` entries stay read-only without exposing their physical template IDs. `runner_group` is an Organization-only scoped custom field. Keep manual refreshes, provider-template loads, mutations, and their follow-up refreshes tied to the query scope that started them.
...
- Before starting claimed queued work, reload the request's persisted profile source and scope, apply the latest effective scope control, and validate current enabled state and requested labels. Admission-time profile data is not sufficient to start a runner after the scope changes.
