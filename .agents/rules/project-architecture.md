...
- Global Runner Specs are admin API/UI data, while `/user/runner-specs` exposes only account or manageable Organization scoped Runner Types. Internal Runner Groups and Repository Policies were removed in Release C: their APIs and state models no longer exist, while legacy tables remain untouched for rollback until a separately authorized cleanup; never use them for admission.
