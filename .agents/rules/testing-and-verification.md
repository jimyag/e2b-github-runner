...
- Run `task ui-production-smoke` after changing UI dependencies, Vite/Rollup configuration, manual chunking, production asset loading, or the Jobs viewport/scroll layout. The smoke must execute the built bundle in Chromium, cover the public landing page, and use local authenticated fixtures to prove that the desktop Jobs list scrolls independently beside the Web Console while narrow layouts retain document flow.
