...
- Ordinary-user Runner Type responses may expose `template_id` and `runner_group` only for the selected scope's own `scoped_custom` entries. Managed entries expose their stable public template name and return the raw scope toggle separately from the effective global-and-scope enabled state; read-only `platform_custom` entries must omit their private template IDs. Organization custom forms may edit `runner_group`; account forms must omit it. Keep async list, template, and mutation follow-up responses bound to the scope that started them so an older account/Organization response cannot overwrite or submit against the current scope.
