...
- `/` is always the public Qiniu CI Runner landing page. `/jobs` is the protected ordinary-user Jobs homepage. Other protected browser routes include `/repositories`, stable job-group routes such as `/github/pulls/{owner}/{repo}/{number}/jobs`, account settings under `/account/repositories`, `/account/preferences`, `/account/sandbox-templates`, and `/account/sandbox-instances`, and the matching `/organizations/{login}/...` routes. Signed-out deep links must preserve their same-origin destination through OAuth `return_to`; unknown routes render 404.
