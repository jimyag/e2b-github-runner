...
- Sandbox service credentials and resource catalogs are ordinary-user account or organization data. Keep credential editing in the account or organization Preferences surface; `/repositories` only shows effective readiness and links there when a manageable scope is missing configuration. Repository-only access can make an organization installation and its authorized repositories visible, but installation-scoped Sandbox mutations require the signed-in GitHub user to be an active organization member; outside collaborators get read-only readiness status. Keep `/user/sandbox/templates` and `/user/sandbox/instances` separate from admin configuration APIs.
