...
- 所选窗口内保持 catalog/Sandbox 配置冻结。readiness 相关变更在审计事件无法持久化时必须失败且不能改变目标数据；一个已经进入写入阶段的变更尝试可能保守地重新开始冻结窗口。备份恢复检查、服务连续运行观察、workflow labels 未修改仍需分别记录人工签字；仅有自动门禁全绿并不等于已授权 Release B。
