...
- 验证 `/account/runner-types` 与一个可管理的 `/organizations/{login}/runner-types` 路由；确认 `/user/runner-specs` 拒绝 repository-only 作用域，只为当前作用域自己的 `scoped_custom` 条目返回 `template_id`，并且即使在可管理 Settings 内也省略平台自定义类型的 template ID。
