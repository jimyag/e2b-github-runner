...
- Active requests recover with bounded concurrency instead of waiting for every earlier request serially; the whole-startup timeout is divided across worker waves so tail requests retain a fair recovery budget.
