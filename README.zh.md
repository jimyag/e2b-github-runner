...
`templates/github-runner-ubuntu-24.04` 是默认 GitHub runner image，包含 runner runtime、Docker support、helper tools 和 `rclone`。`templates/qbox-kodo-ubuntu-16.04` 是额外的 legacy Ubuntu 16.04 template，用于仍需要旧 Go toolchains、apt packages、Docker support 和 `rclone` 的 qbox/kodo-style jobs。它的 Docker base image 定义在 `templates/qbox-kodo-ubuntu-16.04/base.Dockerfile`，可先运行 `task qbox-kodo-base-build` 重建，再重建 Qiniu sandbox template。
